...
import data.scripts.plugins.AI_MissionUIPlugin;
...
public class MissionDefinition implements MissionDefinitionPlugin {
    ...
    private static int globalCurrentMatch;

    private static String playerId, enemyId;
    ...
    public void defineMission(MissionDefinitionAPI api) {
        ...
        try {
            ...
            for (int i = 0; i < matches.length(); i++) {
                ...
                playerId = row.getString("player");
                enemyId = row.getString("enemy");
                int map = row.optInt("map", -1);
                float size = (float) row.optDouble("size", 1f);

                MATCHES.put(2 * i, Match.createPVP(playerId, enemyId, map, size));
                MATCHES.put(2 * i + 1, Match.createPVP(enemyId, playerId, map, size));
            }
        ...
        playerId = currentMatch.sidePlayer;
        Fleet playerFleet = Fleet.createFleet(PATH, playerId);
        ...
        enemyId = currentMatch.sideEnemy;
        Fleet enemyFleet = Fleet.createFleet(PATH, enemyId);
        ...
        if (seed < 0)
            seed = (MissionDefinition.globalCurrentMatch / 2) * playerCost * enemyCost + playerWingNum + enemyWingNum + playerCost * enemyWingNum + playerWingNum * enemyCost;
    ...
    public final static class Plugin extends BaseEveryFrameCombatPlugin {
        ...
        private boolean isEndFlagShown = false;
        private boolean isEndResultShown = false;
        private float mapX;
        private float mapY;
        ...
        public void init(CombatEngineAPI engine) {
            ...
            engine.addPlugin(new AI_MissionUIPlugin(playerId, enemyId));
        ...
        //        SpriteAPI playerP = AI_missionUtils.getPlayerSprite(playerId);
        //        SpriteAPI playerE = AI_missionUtils.getPlayerSprite(enemyId);
        //        SpriteAPI playerPF = AI_missionUtils.getPlayerSprite(playerId);
        //        SpriteAPI playerEF = AI_missionUtils.getPlayerSprite(enemyId);
        ...
        public void advance(float amount, List<InputEventAPI> events) {
            ...
            if (shouldShowEnd) {
                ...
                if (winner != null && !isEndResultShown && timer > 0.5f) {
                    if (GE) {
                        //diplay "END"
